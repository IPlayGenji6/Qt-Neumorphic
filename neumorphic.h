...
public:
    ...
    Neumorphic();
    Neumorphic(NeumorphicMode mode);
    ...
    inline void setOpacity(qreal opacity) { m_opacity = opacity; updateBoundingRect(); }
    inline QPoint offset() const { return m_offset; }
    inline void setOffset(int x, int y) { m_offset = QPoint(x, y); updateBoundingRect(); }
    inline QColor shadowColor() const {return m_shadowColor.toRgb(); }
    inline void setShadowColor(QColor color) {m_shadowColor = color.toRgb(); m_shadowColor.setAlpha(m_opacity); updateBoundingRect(); };
    inline QColor highlightColor() const {return m_highlightColor.toRgb(); }
    inline void setHighlightColor(QColor color) {m_highlightColor = color.toRgb(); m_highlightColor.setAlpha(m_opacity); updateBoundingRect(); };
